...
#define SYS_TASK_STK_SIZE          512//256

static tls_os_queue_t *msg_queue;

#define TLS_SYS_WIFI_AUTO_RECONNECT_ENABLE    1
...
/*
	extern u8 g_hasConnected;
	if (g_hasConnected != 1) {
		printf("[wifi_device]: tls_auto_reconnect Has Not Connected.\n");
		return;
	}
*/
#if 0	
...
#endif	
    int ret = tls_wifi_auto_connect_flag(WIFI_AUTO_CNT_FLAG_GET, &auto_reconnect);
	static u32 reconnectCntr = 0;
	printf("[wifi_device]: tls_auto_reconnect ret=%d, auto_reconnect=%d, reconnectCntr=%d.\n", ret, auto_reconnect, reconnectCntr++);
    if (auto_reconnect == WIFI_AUTO_CNT_OFF)
    {
#if TLS_SYS_WIFI_AUTO_RECONNECT_ENABLE
...
void tls_sys_task(void *data)
{
    ...
    for (;;)
    {
        ...
        if (!err)
        {
            ...
            {
                case SYS_MSG_NET_UP:
                    printf("SYS_MSG_NET_UP\n");
                    sys_net_up();
                    break;
#if TLS_CONFIG_AP
                case SYS_MSG_NET2_UP:
                    printf("SYS_MSG_NET2_UP\n");
                    sys_net2_up();
                    break;
                case SYS_MSG_NET2_DOWN:
                    printf("SYS_MSG_NET2_DOWN\n");
                    sys_net2_down();
                    break;
                case SYS_MSG_NET2_FAIL:
                    printf("SYS_MSG_NET2_FAIL\n");
                    sys_net2_down();
                    tls_auto_reconnect(1);
                    break;
#endif
                case SYS_MSG_NET_DOWN:
                    printf("SYS_MSG_NET_DOWN\n");
                    sys_net_down();
                    break;
                case SYS_MSG_CONNECT_FAILED:
                    printf("SYS_MSG_CONNECT_FAILED\n");
					sys_net_down();
                    break;
                case SYS_MSG_AUTO_MODE_RUN:
                    printf("SYS_MSG_AUTO_MODE_RUN\n");
