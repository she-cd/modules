...
int ble_store_util_get_peer_by_index(int index, ble_addr_t *peer_id_addr)
{
    ...
    memcpy_s(peer_id_addr, sizeof(*peer_id_addr), &peer_id_addrs[index], sizeof(ble_addr_t));
    return rc;
}
...
int ble_store_util_delete_oldest_peer(ble_addr_t *peer_id_addr)
{
    ...
    if (peer_id_addr) {
        memcpy_s(peer_id_addr, sizeof(*peer_id_addr), &peer_id_addrs[0], sizeof(ble_addr_t));
    }
    return 0;
}
