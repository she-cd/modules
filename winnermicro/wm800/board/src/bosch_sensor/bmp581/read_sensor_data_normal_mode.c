...
#include "bmp5.h"
#include "common.h"
struct bmp5_sensor_data bmp581_sensor_data;
...
uint8_t bmp5_dev_addr_test = 0x00;

#include "wm_cpu.h"
int read_sensor_data_normal_mode_bmp581(void)
{
    uint32_t ret;
    uint8_t rslt;
    struct bmp5_dev dev;
     ...
     */
    //bmp5_socket_client_init();
    bmp5_udp_client_init();
    ...
    if (rslt == BMP5_OK)
    {
        ...
        if (rslt == BMP5_OK)
        {
            //ret = bmp5_socket_client_connect();
            ret = bmp5_udp_client_connect();
            if(ret != -1) {
...
#endif
                //bmp5_socket_client_start(&sensor_data);
                ret = bmp5_udp_client_start(&sensor_data);
                if(ret == -1) {
                    printf("udp send data error!\n");
                }
                idx++;
            }
        }
...
SYS_RUN(read_sensor_data_normal_mode_bmp581);