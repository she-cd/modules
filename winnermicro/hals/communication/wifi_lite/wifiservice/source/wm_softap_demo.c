...
int demo_create_softap(u8 *ssid, u8 *key, int chan, int encrypt, int format)
{
    ...
    printf("wifi AP SSID:%s, KEY:%s\r\n", apinfo->ssid, apinfo->keyinfo.key);
...
int Start_softAp()
{
    struct tls_softap_info_t apinfo;
    // 准备AP的配置参数
    strcpy(apinfo.ssid, "ABCD");
    strcpy(apinfo.keyinfo.key, "12345678");

    int test = demo_create_softap(&apinfo.ssid, &apinfo.keyinfo.key, 7, IEEE80211_ENCRYT_CCMP_WPA2, 1);
    printf("%s:%d\n", __func__, __LINE__);
}
//#endif
